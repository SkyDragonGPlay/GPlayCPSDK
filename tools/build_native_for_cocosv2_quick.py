...
def check_quick_environment():
    try:
        QUICK_COCOS2DX_ROOT = os.environ['QUICK_COCOS2DX_ROOT']
    except Exception:
        print "QUICK_COCOS2DX_ROOT not defined. Please define QUICK_COCOS2DX_ROOT in your environment"
        sys.exit(1)
...
def do_build(cocos_root, ndk_root, app_android_root, build_mode, quick_cocos_root):
    ...
    if platform == 'win32':
        ndk_module_path = 'NDK_MODULE_PATH=%s;%s;%s/cocos2dx/platform/third_party/android/prebuilt;%s' % (quick_cocos_root, cocos_root, cocos_root, app_android_root)
    else:
        ndk_module_path = 'NDK_MODULE_PATH=%s:%s:%s/cocos2dx/platform/third_party/android/prebuilt:%s' % (quick_cocos_root, cocos_root, cocos_root, app_android_root)
...
def copy_resources(app_android_root):
    ...
    os.mkdir(assets_dir)
    script_dir = os.path.join(app_android_root, "../scripts")
...
def build(build_mode):
    ...
    ndk_root = check_environment_variables()
    quick_cocos_root = check_quick_environment()
    cocos_root = os.path.join(quick_cocos_root, "lib/cocos2d-x")
    ...
    do_build(cocos_root, ndk_root, app_android_root, build_mode, quick_cocos_root)
