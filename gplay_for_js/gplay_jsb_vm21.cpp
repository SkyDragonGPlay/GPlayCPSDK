...
static const char* s_CPSDKVersion = "GPLAY_CP_SDK_INFO:{\"SDK_VERSION\":\"1.5.2\",\"ENGINE\":\"COCOS_JS\",\"ENGINE_BASE_VERSION\":\"V2\",\"SDK_VERSION_NAME\":\"1.5.2 release 2\"}";
...
static void actionResultCallback(int result, const char* jsonResult, int callbackId)
{
    ...
    if ( s_asyncJSCallbackFunc == JSVAL_VOID ) {
        ...
        valArr[1] = c_string_to_jsval(cx, jsonResult);
...
static JSBool jsb_gplay_setAsyncActionResultListener(JSContext *cx, uint32_t argc, jsval *vp)
{
...
static JSBool jsb_gplay_setPreloadResponseCallback(JSContext *cx, uint32_t argc, jsval *vp)
{
    if (argc == 2)
    {
        jsval *argv = JS_ARGV(cx, vp);
...
static JSBool jsb_gplay_init(JSContext* cx, uint32_t argc, jsval* vp)
{
...
static JSBool jsb_gplay_isInGplayEnv(JSContext *cx, uint32_t argc, jsval *vp)
{
    jsval *argv = JS_ARGV(cx, vp);
...
static JSBool jsb_gplay_isDivideResourceMode(JSContext *cx, uint32_t argc, jsval *vp)
{
    jsval *argv = JS_ARGV(cx, vp);
...
static JSBool jsb_gplay_preloadGroups(JSContext *cx, uint32_t argc, jsval *vp)
{
...
static JSBool jsb_gplay_noticePreloadMultipleGroup(JSContext *cx, uint32_t argc, jsval *vp)
{
...
static JSBool jsb_gplay_noticePreloadBegin(JSContext *cx, uint32_t argc, jsval *vp)
{
    jsval *argv = JS_ARGV(cx, vp);
...
static JSBool jsb_gplay_noticePreloadEnd(JSContext *cx, uint32_t argc, jsval *vp)
{
    jsval *argv = JS_ARGV(cx, vp);
...
static JSBool jsb_gplay_getNetworkType(JSContext *cx, uint32_t argc, jsval *vp)
{
...
// jsb_gplay.quitGame()
static JSBool jsb_gplay_quitGame(JSContext *cx, uint32_t argc, jsval *vp)
{
...
static JSBool jsb_gplay_isLogon(JSContext* cx, uint32_t argc, jsval* vp)
{
...
static JSBool jsb_gplay_getUserID(JSContext* cx, uint32_t argc, jsval* vp)
{
    CCLOG("jsb_gplay_getUserID()");

    jsval *argv = JS_ARGV(cx, vp);
    const std::string& userId = gplay::common::getUserID();
    JS_SET_RVAL(cx, vp, c_string_to_jsval(cx, userId.c_str()));
    return true;
}

static JSBool jsb_gplay_getOrderId(JSContext* cx, uint32_t argc, jsval* vp)
{
    CCLOG("jsb_gplay_getOrderId()");
    jsval *argv = JS_ARGV(cx, vp);
    
    const std::string& orderId = gplay::common::getOrderId();
    JS_SET_RVAL(cx, vp, c_string_to_jsval(cx, orderId.c_str()));
    return true;
}

static JSBool jsb_gplay_getChannelId(JSContext* cx, uint32_t argc, jsval* vp)
{
    CCLOG("jsb_gplay_getChannelId()");
    jsval *argv = JS_ARGV(cx, vp);
    
    const std::string& channelId = gplay::common::getChannelID();
    JS_SET_RVAL(cx, vp, c_string_to_jsval(cx, channelId.c_str()));
    return true;
}

static JSBool jsb_gplay_isSupportingFunc(JSContext* cx, uint32_t argc, jsval* vp)
{
...
static JSBool jsb_gplay_login(JSContext* cx, uint32_t argc, jsval* vp)
{
...
static JSBool jsb_gplay_share(JSContext* cx, uint32_t argc, jsval* vp)
{
    ...
    if (argc == 2)
    {
        ...
        if (jsval_to_int(cx, argv[0], &callbackId) &&
            jsval_to_std_string(cx, argv[1], &param) )
        {
...
static JSBool jsb_gplay_pay(JSContext* cx, uint32_t argc, jsval* vp)
{
    CCLOG("jsb_gplay_pay()");
    jsval *argv = JS_ARGV(cx, vp);
...
static JSBool jsb_gplay_createShortCut(JSContext* cx, uint32_t argc, jsval* vp)
{
...
static JSBool jsb_gplay_callSyncFunc(JSContext* cx, uint32_t argc, jsval* vp)
{
...
static JSBool jsb_gplay_callAsyncFunc(JSContext* cx, uint32_t argc, jsval* vp)
{
...
void register_gplay_runtime(JSContext* cx, JSObject* global)
{
    ...
    JSObject *jsbGplayObj = JS_NewObject(cx, NULL, NULL, NULL);
    jsval jsbGplayVal = OBJECT_TO_JSVAL(jsbGplayObj);
